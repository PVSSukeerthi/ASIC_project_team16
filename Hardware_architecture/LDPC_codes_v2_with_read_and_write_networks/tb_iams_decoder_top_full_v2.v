...
module tb_iams_decoder_top_full;
    ...
    // All-zeros codeword with strong LLR (+40) except one bit flipped
    // to a WEAK positive (+2) — still votes for bit=0 but barely.
    // This is well within the decoder's correction capability.
    // (Using a negative LLR risks failing if H matrix rows 4-41 are
    //  not a complete real code — a weak positive is safer for testing.)
        ...
        begin
            ...
            for (row = 0; row < BG_COLS; row = row + 1) begin
                for (col = 0; col < Z; col = col + 1) begin
                    llr_wr_addr = row * Z + col;
                    if (row == 0 && col == 0)
                        llr_wr_data = 8'sd2;   // weak positive: "probably 0" but uncertain
                    else
                        llr_wr_data = 8'sd40;  // strong positive: confident 0
                    @(posedge clk); #1;
                end
            end
            llr_wr_en = 0;
    ...
    initial begin
        ...
        // =====================================================================
        // TEST 2: Single uncertain bit (weak LLR at one position)
        // All other bits: +40, one bit at (col=0,z=0): +2 (uncertain)
        // Decoder should converge and satisfy all parity checks.
        // =====================================================================
        $display("\n[TEST 2] Uncertain bit at (col=0, z=0) — LLR=+2 vs +40 elsewhere");
        ...
        check("T2: syndrome_ok (parity satisfied)", syndrome_ok);
        $display("  Iterations used: %0d  syndrome=%0b", iter_count, syndrome_ok);
    ...
    initial begin
        ...
        // Dump ALL signals in testbench AND one level into DUT
        $dumpvars(0, tb_iams_decoder_top_full);        // TB signals
        $dumpvars(1, tb_iams_decoder_top_full.dut);    // DUT top-level signals
        $dumpvars(1, tb_iams_decoder_top_full.dut.u_ctrl);   // Controller FSM
        $dumpvars(1, tb_iams_decoder_top_full.dut.u_cnu);    // CNU Z-parallel
        $dumpvars(1, tb_iams_decoder_top_full.dut.u_syn);    // Syndrome checker
        $dumpvars(1, tb_iams_decoder_top_full.dut.u_hrom);   // H-matrix ROM
        $dumpvars(1, tb_iams_decoder_top_full.dut.u_rn);     // Read network
        $dumpvars(1, tb_iams_decoder_top_full.dut.u_wn);     // Write network
    end
